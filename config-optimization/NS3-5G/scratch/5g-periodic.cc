...
main (int argc, char *argv[])
{
  ...
  int numEnb = 2;
  int numSta = 10; // Including Probing node
  double simulationTime = 2.5;
  double interPacketInterval = 0.0057;
  std::string trafficDirection = "upstream";
  double payloadSize = 1500;
  double distance = 30.0; // eNB-UE distance in meters
  std::string radioEnvironment = "urban";

  bool harqEnabled = true;
  bool rlcAmEnabled = true;
  double voltage = 3; // in W
  double batteryCap = 2400; // Battery capacity in mAh
  ...
  CommandLine cmd;
  cmd.AddValue ("numSta", "Number of UEs per eNB", numSta);
  ...
  Config::SetDefault ("ns3::ThreeGppChannelModel::UpdatePeriod", TimeValue(MilliSeconds (0))); // never update the channel
  Config::SetDefault ("ns3::ThreeGppChannelConditionModel::UpdatePeriod", TimeValue(MilliSeconds (0.0))); // never update the channel condition
  ...
  */
  ...
  if (radioEnvironment == "indoor") {
      ...
      if (latency) {
        Ptr<ListPositionAllocator> probingAlloc = CreateObject<ListPositionAllocator> ();
        probingAlloc->Add (Vector (1, 1, 1));
        mobility.SetPositionAllocator (probingAlloc);
        mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  ...
  */
  ...
  std::cout << "Success rate: " <<  wholeSuccessRate;
