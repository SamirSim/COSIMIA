...
int main (int argc, char *argv[]) {
  ...
  // Radion environment
  std::string radioEnvironment = "suburban";
  ...
  if (radioEnvironment == "urban") channel.AddPropagationLoss("ns3::"+propLoss, "Frequency", DoubleValue(frequency), "Lambda", DoubleValue(300000000.0/frequency));
    ...
    channel.AddPropagationLoss ("ns3::"+propLoss, "Frequency", DoubleValue(frequency));
  }
  ...
  if (hiddenStations) {
    ...
    if (radioEnvironment == "indoor") {
      ...
      if (latency) {
        Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator> ();
        positionAlloc->Add (Vector (5, 0, 1));
        mobility.SetPositionAllocator (positionAlloc);
        mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
        mobility.Install (wifiProbingNode);
      }
  ...
  if (latency) {
    ...
    uint32_t payloadSizeEcho = 50; //Packet size for Echo UDP App
